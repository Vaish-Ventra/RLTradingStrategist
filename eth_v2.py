...
df = pd.read_csv(r'C:\Users\Shambhavi S Vijay\OneDrive\Desktop\proj\predicted_ETH.csv')
...
class Strat_1(Strategy):
    ...
    def init(self):
        ...
        self.vol_sma_fast = self.I(lambda x: SMAIndicator(pd.Series(x), window=self.volume_sma_fast_period).sma_indicator(), volume)
        self.vol_sma_slow = self.I(lambda x: SMAIndicator(pd.Series(x), window=self.volume_sma_slow_period).sma_indicator(), volume)
    ...
    def next(self):
        ...
        cond_long_2 = self.vol_sma_fast[i] > self.vol_sma_slow[i]
...
# Using bt.optimize to get best combination of parameters
bt = Backtest(df, Strat_1, cash=100_000, commission=0.0015, exclusive_orders=True)

stats = bt.optimize(
    rsi_up=range(65, 85, 5),
    ema_fastest_period=range(5, 15, 2),
    ema_fast_period=range(15, 35, 5),
    ema_slow_period=range(45, 75, 5),
    trailing_pct=[0.01, 0.015, 0.02, 0.025, 0.03],
